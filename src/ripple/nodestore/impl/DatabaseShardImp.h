...
private:
    ...
    std::uint32_t
    getShardBoundaryIndex(std::lock_guard<std::mutex> const&);

    std::uint32_t
    countHistoricalShards(std::lock_guard<std::mutex> const&);
    ...
    void
    checkRecentShards(std::lock_guard<std::mutex> const& lock);

    // Checks whether the shard can be stored. If
    // the new shard can't be stored, returns
    // boost::none. Otherwise returns a bool
    ...
    boost::filesystem::path
    chooseHistoricalPath(std::lock_guard<std::mutex> const&);
};
